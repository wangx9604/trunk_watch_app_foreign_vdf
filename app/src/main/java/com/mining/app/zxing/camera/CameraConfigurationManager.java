...
final class CameraConfigurationManager {
  ...
  private static final int TEN_DESIRED_ZOOM = 17;
...
//    cameraResolution.y=height;
  ...
  private static Point findBestPreviewSizeValue(CharSequence previewSizeValueString, Point screenResolution) {
    ...
    for (String previewSize : COMMA_PATTERN.split(previewSizeValueString)) {
      ...
      float newProportionDiff = Math.abs(((float)screenResolution.x/(float)screenResolution.y) - ((float)newX/(float)newY));
      ...
      if (newDiff == 0) {
        ...
        if (newProportionDiff<proportionDiff){
          bestX = newX;
          bestY = newY;
          diff = newDiff;
          proportionDiff = newProportionDiff;
        } else if (newProportionDiff == proportionDiff) {
          if (newDiff < diff) {
            bestX = newX;
            bestY = newY;
            diff = newDiff;
