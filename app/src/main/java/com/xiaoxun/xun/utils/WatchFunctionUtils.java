...
public class WatchFunctionUtils {
    ...
    public static void clickVideoCall(ImibabyApp app, Activity activity, WatchData watch) {
        ...
        LogUtil.e("clickVideoCall WatchData.getVideoCallVersion(watch) = " + WatchData.getVideoCallVersion(watch));
        String BgActivityName = VideoCallActivity.class.getName();
        if (WatchData.getVideoCallVersion(watch) == 2) {
            BgActivityName = VideoCallActivity2.class.getName();
            intent.setClass(activity, VideoCallActivity2.class);
        }else if (WatchData.getVideoCallVersion(watch) == 3) {
            BgActivityName = VideoCallActivity3.class.getName();
            intent.setClass(activity, VideoCallActivity3.class);
        }
        intent.putExtra(Const.VIDEOCALL_TYPE, 0);
        intent.putExtra(CloudBridgeUtil.KEY_NAME_EID, watch.getEid());
        activity.startActivity(intent);
//        BgStart.getInstance().startActivity(activity, intent, BgActivityName);
    }
