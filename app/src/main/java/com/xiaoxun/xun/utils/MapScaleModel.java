...
public class MapScaleModel {
    ...
    MapScaleModel(float density, final Context context) {
    ...
    private String text(int distance, boolean meters) {
        if (meters) {
            if (distance < 1000) {
                return distance + ctxt.getResources().getString(R.string.unit_meter);
            }
            else {
                return distance / 1000 + ctxt.getResources().getString(R.string.unit_kilometer);
            }
        } else {
