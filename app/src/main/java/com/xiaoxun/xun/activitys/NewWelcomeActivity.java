...
import com.xiaoxun.xun.region.PrivacyAuthDialog;
...
//                    ToMainActivity();
    ...
    private void getCheckBindCode() {
        ToMainActivity();
    }

    private void toPrivacyAuthActivity() {
        boolean needAutoLogin = myApp.isAutoLogin();
        Log.i("cui", "needAutoLogin =" + needAutoLogin);
        if (needAutoLogin) {
            getCheckBindCode();
            //gotoMainActivity();
        } else {
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    if (myApp.getBoolValue(Const.SHARE_PREF_PRIVACY_POLICY_AGREED, false)) {
                        if (myApp.getIntValue(Constants.KEY_NAME_COUNTRY_SELECTED, 3) == 3) {
                            Intent intent = new Intent(NewWelcomeActivity.this, RegionSelectActivity.class);
...
//                            finish();
                        ...
                        Dialog dialog = new PrivacyAuthDialog(NewWelcomeActivity.this);
                        dialog.show();
                    }
                }
            }, 2000);
