...
public class NormalNoticeItemHolder extends RecyclerView.ViewHolder implements RecyclerItemHolder {
    ...
    public void bindTo(final NoticeMsgData msg, String time) {
        ...
        } else if (msg.getmType() == NoticeMsgData.MSG_TYPE_SYSTEM) {
            mIconView.setBackgroundResource(R.drawable.set_up_icon);
            JSONObject systemJson = (JSONObject) JSONValue.parse(msg.getmContent());
            String title = (String) systemJson.get(CloudBridgeUtil.KEY_NAME_TITLE);
            String text = (String) systemJson.get(CloudBridgeUtil.E2C_PL_KEY_TYPE_TEXT);
