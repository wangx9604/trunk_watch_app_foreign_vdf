 ...
 */
package com.xiaoxun.xun.views.indoorview;
...
public class BaseStripAdapter extends BaseAdapter {
    ...
    public int getPostionFromFloor(String floor){
        ...
        for(int i=0;i<getCount();i++){
            if(floor.equalsIgnoreCase(mFloorList.get(i))){
                pos = i;
                break;
            }
        }
        return mFloorList.size()-1-pos;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        String floor = mFloorList.get(mFloorList.size() - 1 - position);
        if (floor != null) {
