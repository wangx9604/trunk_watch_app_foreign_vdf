...
public class VoiceFloatingView extends View {
    ...
    private void init() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            mLayoutParams.type = WindowManager.LayoutParams.TYPE_APPLICATION_OVERLAY;  //实现在其他应用和窗口上方显示提醒窗口
        } else {
            mLayoutParams.type = WindowManager.LayoutParams.TYPE_PHONE;   //表示提供用户交互操作的非应用窗口
        }
        ...
        mBitmap = ((BitmapDrawable) getResources().getDrawable(R.drawable.video_call_normal)).getBitmap();
        mDefaultHeight = 210;
        mDefaultWidth = 210;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        switch (mDirection) {
            ...
            case right:
                ...
                canvas.drawRoundRect(mWidth >> 1, 0, mWidth, mHeight, 0, 0, mPaint);
                break;
            case left:
                mPaint.setXfermode(mPorterDuffXfermode);
                canvas.drawRoundRect(0, 0, mWidth >> 1, mHeight, 0, 0, mPaint);
                break;
            case move:
                break;
        }
        ...
        //居中填充icon
        canvas.drawBitmap(mBitmap, (mWidth - mBitmap.getWidth()) >> 1, (mHeight - mBitmap.getHeight()) >> 1, mPaint);
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (upTime == 0 || (upTime - downTime) < 300)
