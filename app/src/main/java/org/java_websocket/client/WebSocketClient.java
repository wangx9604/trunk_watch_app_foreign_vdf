 ...
 * useful. At runtime the user is expected to establish a connection via {@link #connect()}, then receive events like {@link #onMessage(String, int)} via the overloaded methods and to {@link #send(String)} data to the server.
 */
public abstract class WebSocketClient extends WebSocketAdapter implements Runnable, WebSocket {
	...
	public void send( String text ) throws NotYetConnectedException {
		engine.send( text );
        LogUtil.oo_sdcardLog("sendmessage engine.getLocalSocketAddress(): " + engine.getLocalSocketAddress()+" content"+text);
	}
	...
	public void run() {
		...
		try {
			...
			LogUtil.oo_sdcardLog("WebSocketClient after engine.decode isclosed:"+isclosed+"  readbytes"+readBytes);
			engine.eot();
		} catch ( IOException e ) {
            ...
            if (getLocalSocketAddress() != null) {
                port = getLocalSocketAddress().getPort();
            }
			onError( e, port);
	...
	public final void onWebsocketMessage( WebSocket conn, String message ) {
		onMessage( message ,conn.getLocalSocketAddress().getPort());
        LogUtil.oo_sdcardLog("WebSocketClient onMessage: " + conn.getLocalSocketAddress()+"  content:"+message);
	}
	...
	public final void onWebsocketOpen( WebSocket conn, Handshakedata handshake ) {
		...
		onOpen( (ServerHandshake) handshake ,conn.getLocalSocketAddress().getPort());
	}
	...
	public final void onWebsocketClose( WebSocket conn, int code, String reason, boolean remote ) {
        ...
        int port = 0;
        if (conn != null && conn.getLocalSocketAddress() != null) {
            port = conn.getLocalSocketAddress().getPort();
        }

		onClose( code, reason, remote, port);
	}
	...
	// ABTRACT METHODS /////////////////////////////////////////////////////////
	public abstract void onOpen( ServerHandshake handshakedata,int port );
	public abstract void onMessage( String message,int port );
	public abstract void onClose( int code, String reason, boolean remote, int port);
	public abstract void onError( Exception ex, int port);
