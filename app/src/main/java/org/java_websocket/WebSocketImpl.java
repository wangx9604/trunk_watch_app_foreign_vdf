...
public class WebSocketImpl implements WebSocket {
	...
	public void decode( ByteBuffer socketBuffer ) {
		...
		if( DEBUG )
			LogUtil.i("process(" + socketBuffer.remaining() + "): {" + (socketBuffer.remaining() > 1000 ? "too big to display" : new String(socketBuffer.array(), socketBuffer.position(), socketBuffer.remaining())) + "}");
	...
	public void sendFrame( Framedata framedata ) {
		if( DEBUG )
			LogUtil.i("send frame: " + framedata);
	...
	private void write( ByteBuffer buf ) {
		if( DEBUG )
			LogUtil.i("write(" + buf.remaining() + "): {" + (buf.remaining() > 1000 ? "too big to display" : new String(buf.array())) + "}");
	...
	private void open( Handshakedata d ) {
		if( DEBUG )
			LogUtil.i("open using draft: " + draft.getClass().getSimpleName());
